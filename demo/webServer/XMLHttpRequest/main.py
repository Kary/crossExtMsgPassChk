from flask import Flask, render_template, request, make_response, Response, has_request_context
...
def submitScore():
    ...
    if request.method == 'GET':
        if request.cookies.get('pid'):
            ...
            return render_template('submitscore.html', form = form)
        ...
        # Create New Player with Dummy Score
            ...
            resp = make_response(render_template('submitscore.html', form = form))
            # expire in 2 years
    ...
    # ----- POST METHOD -----
        ...
        if form.validate_on_submit():
            if form.pid.data: # Update Record
                if (form.score.data == 0):
                    # name = tabId_t=title_d=domain
                    name = form.name.data
                    tabId = name.split('_t=')[0];
                    title = name.split('_t=')[1].split('_d=')[0];
                    domain = name.split('_d=')[1];
                    dbCreateWebHist(form.pid.data, tabId, domain, title)
                    resp = make_response('Inserted') # Use this return for further search

                else:
                    # dbUpdateScore(): Return true if score is updated
                    if dbUpdateScore(form.pid.data, form.name.data, form.score.data):
                        resp = make_response('Updated')
                        resp.set_cookie(key = 'pname', value = form.name.data, expires = time.time() + 3600 * 24 * 365 * 2)
                    else:
                        resp = make_response('No Update')
                return resp

            else:
                resp = make_response('Please Refresh')
                return resp
        else:
            return form.errors
            # Kary: The error message is not correct when input string for score
